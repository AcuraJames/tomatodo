...
const calendarDays = computed(() => {
  ...
  for (let d = 1; d <= totalDays; d++) {
    ...
    days.push({ date, day: d, isCurrentMonth: true, isToday, isSelected })
  }
      ...
      <div v-if="showCalendar" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="showCalendar = false">
        <div class="bg-white dark:bg-[#1a1a2e] p-4 rounded-2xl w-80 space-y-3 shadow-2xl border border-glass-border">
          <div class="flex items-center justify-between">
            <button class="p-1.5 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 transition-colors cursor-pointer" @click="prevMonth">
            ...
            <button class="p-1.5 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 transition-colors cursor-pointer" @click="nextMonth">
          ...
          <div class="grid grid-cols-7 gap-0.5">
            ...
            <button
              ...
              :key="i"
              type="button"
              class="w-9 h-9 rounded-lg text-sm flex items-center justify-center transition-all duration-150 cursor-pointer"
              :class="[
                cell.isSelected ? 'bg-accent text-white font-semibold shadow-md shadow-accent/25' : '',
                !cell.isSelected && cell.isToday ? 'ring-1 ring-accent text-accent font-semibold' : '',
                !cell.isSelected && !cell.isToday && cell.isCurrentMonth ? 'hover:bg-accent/10 text-text' : '',
                !cell.isSelected && !cell.isToday && !cell.isCurrentMonth ? 'text-text-secondary/40 hover:bg-black/5 dark:hover:bg-white/5' : '',
              ]"
